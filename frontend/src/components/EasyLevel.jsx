...
const EasyLevel = () => {
    ...
    const handleOptionClick = (option) => {
        ...
        if (option === easyQuestions[currentIndex].correctAnswer) {
            setIsCorrect(true);
            setStars(stars + 1);
            setTimeout(() => {
                setIsCorrect(false);
    ...
    const handleNext = () => {
        if (currentIndex < easyQuestions.length - 1) {
            setCurrentIndex(currentIndex + 1);
            setSelectedOption("");
        } else {
            navigate("/sand-writing");
        }
    };
        ...
        <div className="bg-white min-h-screen p-6 text-center">
            ...
            <div className="w-full bg-gray-300 rounded-full h-4 mb-6 shadow-inner">
                <div
                    ...
                    style={{ width: `${((currentIndex + 1) / easyQuestions.length) * 100}%` }}
                ></div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-xl border-4 border-blue-400 relative w-80 h-80 mx-auto mb-6">
                <img
                    src={easyQuestions[currentIndex].image}
                    alt="Question"
